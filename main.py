...
class MainWindow(QtWidgets.QMainWindow, Ui_CRPmain):
    ...
    def process_name_file(self, file_path):
        ...
        print("读取到的有效名单长度:", state.namelen)
        if state.non_repetitive == 1:
        ...
        if num > state.namelen:
            ...
            if state.mrunning == False:
                ...
                try:
                    ...
                    log_print("无法写入历史记录")
                print(state.today, "幸运儿是： %s " % name_set)
            ...
            if state.non_repetitive == 1:
                if len(state.non_repetitive_list) > 0:
                    if state.origin_name_list in state.non_repetitive_list:
                        state.non_repetitive_list.remove(state.origin_name_list)
                    else:
                        print(f"不重复的单抽名单中没有{state.origin_name_list}")
                if state.namelen != 0:
        ...
        if state.running:  # 结束按钮
            ...
            if state.inertia_roll == 1:
                if len(state.non_repetitive_list) == 1 or len(state.name_list) == 0:
                    ...
                    s = 50
                    speed = random.randint(state.roll_speed-30, state.roll_speed+30)
                    while speed <= 650:
            ...
            if len(state.name_list) != 0:
                if state.bgmusic == 1:
                    ...
                    try:
                        ...
                        print("无法播放音乐文件：%s，错误信息：%s" % (state.file_path, e))
                        self.signals.update_pushbotton.emit(_(" 结束"), 2)
                        self.signals.enable_button.emit(4)
